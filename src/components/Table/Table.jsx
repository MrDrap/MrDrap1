...
function Table({ columns, data, onRowClick, actions = [], itemsPerPage, selectable = false, clickableCondition = () => true}) {
      ...
      <table className="table">
        ...
        <tbody>
          { items.map((item, itemIndex) => (
              <tr className={ `${ (onRowClick && clickableCondition(item)) ? 'clickable' : '' } ${ selectable && (itemIndex === parseInt(selectedRowIndex) ? 'selected' : '') }` } key={ `${ itemIndex }-tr` }>
                { Object.values(item).map((value, index) => {
                    return columns[index].subColumns 
                      ? columns[index].subColumns.map((subColumn, innerIndex) => 
                          <td onClick={ clickableCondition(item) ? handleRowClick : undefined } key={ `${ innerIndex }-td-sub` } style={ columns[index].subColumns[innerIndex].styles ? (columns[index].subColumns[innerIndex].styles.condition(value) ? columns[index].subColumns[innerIndex].styles.style : {}) : {}}> 
                            { subColumn.format ? subColumn.format(subColumn.accessor(value)): subColumn.accessor(value) } 
                          </td>
                        )
                        
                      : <td onClick={ clickableCondition(item) ? handleRowClick : undefined } key={ `${ index }-td` } style={ columns[index].styles ? (columns[index].styles.condition(value) ? columns[index].styles.style : {}) : {}} id={ itemIndex }> { columns[index].format ? columns[index].format(value): value } </td>
